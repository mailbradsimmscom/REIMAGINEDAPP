...
export async function upsertDoc({ id, system_id, knowledge_type, title, content, source, tags, version }) {
  ...
  if (error) throw new DatabaseError('Failed to upsert document', 'upsert', TABLE, error);
  return data; // includes id
}

export async function getDoc(id) {
  const { data, error } = await supabase.from(TABLE).select('*').eq('id', id).single();
  if (error) throw new DatabaseError('Failed to get document', 'select', TABLE, error);
  return data;
}

export async function updateDocVersionAndContent(id, patch) {
  const { data, error } = await supabase.from(TABLE).update(patch).eq('id', id).select().single();
  if (error) throw new DatabaseError('Failed to update document', 'update', TABLE, error);
  return data;
}
...
export async function softDeleteDoc(id) {
  ...
  if (error) throw new DatabaseError('Failed to soft delete document', 'update', TABLE, error);
  return data;
}
